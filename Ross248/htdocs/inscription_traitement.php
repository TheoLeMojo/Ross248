<?php 
    ...
    if(!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['password_retype']))
    {
        ...
        $check = $bdd->prepare('SELECT name, email, password FROM users WHERE email = ?');
        ...
        if($row == 0){ 
            if(strlen($name) <= 100){ // On verifie que la longueur du name <= 100
                    ...
                    if(filter_var($email, FILTER_VALIDATE_EMAIL)){ // Si l'email est de la bonne forme
                        if($password === $password_retype){ // si les deux mdp saisis sont bon
                            ...
                            */
                            ...
                            $insert = $bdd->prepare('INSERT INTO utilisateurs(name, email, password, ip, token) VALUES(:name, :email, :password, :ip, :token)');
